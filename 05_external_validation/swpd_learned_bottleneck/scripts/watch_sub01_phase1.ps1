[CmdletBinding()]
param(
    [string]$LauncherReceipt = 'C:\WhisperECoG_Work\SWPD\runs\learned_bottleneck_sub01_phase1_v1\launcher\launcher.json',
    [switch]$Follow
)
...
if (Test-Path -LiteralPath $QueueState -PathType Leaf) {
    Get-Content -LiteralPath $QueueState -Raw -Encoding UTF8
}
